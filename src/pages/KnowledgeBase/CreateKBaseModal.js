...
import { toggleModalCreateKbase, postKbases } from 'store/actions'
...
const CreateKBaseModal = () => {
    ...
    const showModal = useSelector(state => state.kbase.modalCreateKbase)
    const showCreateLoadPanel = useSelector(state => state.kbase.loadingCreate)
    ...
    const onKBaseSubmit = () =>{
          ...
          if( refValidTitle ){
            fileUpRef.current.instance._files.forEach(item => {
              ...
              dispatch(postKbases(dataSubmit, appActive, moduleActive))
          }
    }
    ...
    return (
        <>
               <Modal isOpen={showModal} backdrop={'static'} role="dialog" autoFocus={true} centered={true} size={'xl'} className="createKBaseModal" tabIndex="-1" toggle={() => { dispatch(toggleModalCreateKbase(!showModal)) }} >
                <div className="modal-content">
                  <ModalHeader toggle={() => { dispatch(toggleModalCreateKbase(!showModal)) }} > New Knowledge Base </ModalHeader>
                  ...
                  <ModalFooter style={{paddingTop:'0.25rem', paddingBottom:'0.25rem'}}>
                    <Button type="button" color="secondary" onClick={() => { dispatch(toggleModalCreateKbase(!showModal)) }} >
                      Close
                    </Button>
...
export default React.memo(CreateKBaseModal)

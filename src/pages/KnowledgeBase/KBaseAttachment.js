...
import ppt from '../../assets/images/file_format/ppt.png'
import config from '../../config'
import getImgUrl from '../../helpers/url_image_helper'

function KBaseAttachment({data, folder, openImage, setImage, freeToken}) {
    return (
        <>
            {
                data &&  map(data, (atc, index)=> {
                    if(atc.fileType === '.pdf'){
                        return  (
                            <Col className={"col-6 col-sm-2 col-md-2 col-xl-2"} key={`atchment ${index}`}>
                                <Card style={{marginBottom:'5px'}}>
                                    ...
                                    {/* <a href={`${config.apiURL}media/${folder}/download/${atc.id}`} className="fw-medium"> Download </a> */}
                                        <a href={ getImgUrl(folder, true, atc.id, freeToken) } className="fw-medium"> Download </a>
                                    </div>
                                </Card>
                            </Col>
                        )
                        ...
                        return  (
                            <Col className={"col-md-2 col-xl-2"} key={`atchment ${index}`}>
                                <Card style={{marginBottom:'5px'}}>
                    ...
                    else if ( atc.fileType === '.ppt' || atc.fileType === '.pptx' ){
                        return  (
                            <Col className={"col-6 col-sm-2 col-md-2 col-xl-2"} key={`atchment ${index}`}>
                                <Card style={{marginBottom:'5px'}}>
                                    <img className="card-img-top img-fluid img-thumbnail p-2" src={ppt} alt="ppt" />
                                    <div className="py-2 text-center">
                                        <a href={getImgUrl(folder, true, atc.id, freeToken)} className="fw-medium"> Download </a>
                                    </div>
                                </Card>
                            ...
                            <Col className={"col-6 col-sm-2 col-md-2 col-xl-2"} key={`atchment ${index}`}>
                                <Card style={{marginBottom:'5px'}}>
                                    <img 
                                     ...
                                     src={getImgUrl(folder, false, atc.id, freeToken)}
                                     alt="image" 
                                     style={{cursor:'pointer'}} 
                                     onClick={() => {
                                        openImage(true)
                                        setImage(getImgUrl(folder, false, atc.id, freeToken))
                                     }}
                                    />
                                    <div className="py-2 text-center">
                                         <a href={getImgUrl(folder, true, atc.id, freeToken)} className="fw-medium"> Download </a>
...
export default React.memo(KBaseAttachment)
